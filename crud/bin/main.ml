...
in
  let prefix_label = label "Filter Prefix:" in
  let p_label_layout = L.resident prefix_label 
in
  let prefix_input = text_input ~prompt: " " () in
...
in
  let name_input = text_input ~prompt:"Enter name" () in
  let n_input_layout = L.resident ~w:150 prefix_input ~background:(Layout.style_bg style)
in
  let name_layout = L.flat [n_label_layout; n_input_layout;] 
in
  let surname_label = label "Surname: " in
  let s_label_layout = L.resident surname_label
in
  let surname_input = text_input ~prompt:"Enter Surname" () in
  let s_input_layout = L.resident ~w:150 prefix_input ~background:(Layout.style_bg style)
in
...
in
  let create_button = button "Create" in
  let _ = on_click ~click: (fun _ ->
    let name = get_text name_input in
    let surname = get_text surname_input in
    let full_name = name ^ " " ^ surname in
    names := full_name :: !names;
    set_text box (String.concat ", " !names);
    (* W.set_text name_input "";
    W.set_text surname_input ""; *)
  ) create_button 
in
  let update_button = button "Update" in
  let _ = on_click ~click: (fun _ ->
    let name = get_text name_input in
    let surname = get_text surname_input in
    let full_name = name ^ " " ^ surname in
...
in
  let delete_button = button "Delete" in
  let _ = on_click ~click: (fun _ ->
    let name = get_text name_input in
    let surname = get_text surname_input in
    let full_name = name ^ " " ^ surname in
