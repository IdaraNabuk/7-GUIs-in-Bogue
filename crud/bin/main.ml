...
in
  ...
  let p_input_layout = L.resident ~w:100 prefix_input ~background:(Layout.style_bg style)
in
  let prefix_layout = L.flat [p_label_layout; p_input_layout]
in
  let list_widget = W.text_display "" in
  let box_display = L.resident ~w:180 ~h:180 list_widget ~background:(Layout.style_bg style)
in
  let box_layout = L.flat [box_display]
in
...
in
  ...
  let s_input_layout = L.resident ~x:10 ~w:150 surname_input ~background:(Layout.style_bg style)
in
  let surname_layout = L.flat [s_label_layout; s_input_layout;] in
  let names_layout = L.tower [ name_layout; surname_layout] 
in
  let naming = L.flat ~margins:5 [box_layout; names_layout] 
in
...
let name = W.get_text name_input in
    ...
    W.set_text list_widget (String.concat ", \n" !names);
...
in
  ...
  let _ = W.on_click ~click: (fun _ ->
    ...
    W.set_text list_widget (String.concat ", " !names);
  ) update_button 
in
...
in
  let button_layout = L.flat_of_w [create_button; update_button; delete_button;]
in
